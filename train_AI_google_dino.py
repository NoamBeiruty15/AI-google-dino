...
class Game:
        ...
        if self.obstacle_spawn_ticker >= random.randint(45, 200):
            ...
            if self.score >= self.min_score_birds_start_spawning:
                if random.randint(1, 100) <= 75:
                    ...
                    self.spawn_bird()
            else:  # Spawn only cacti if the score is low
    ...
    def update(self):
        ...
        if not self.is_dino_saved:
            if self.score >= 5000:
                if self.genomes:
                    genome_data = self.genomes[0]
                    with open("trained_dino_AI.pkl", "wb") as f:
